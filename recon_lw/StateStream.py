...
def get_next_update_oe(m):
    ...
    ord_status = m['body']['fields']["OrdStatus"] if is_fix else str(m['body']['fields']["OrderStatus"])
    ord_type = m['body']['fields']["OrdType"] if is_fix else str(m['body']['fields']["OrderType"])
    target_comp_id = m['body']['fields']["header"]["TargetCompID"] if is_fix else str(m['body']['fields']["CompID"])
...
def md_oe_state_stream_compare(match , custom_settings, create_event, save_events):
    if match[1] is None:
        error_event = create_event("OEStateNotFound", "OEStateNotFound", False, {"md_event": match[0]})
    ...
    result_event = create_event("MD_L3_vs_OE_Check",
                                "MD_L3_vs_OE_Check",
...
def create_oe_snapshots_streams(oe_streams, result_events_path):
    ...
    buffer_len = 100
    while len(streams) > 0:
        next_batch_len = recon_lw.get_next_batch(streams, message_buffer, buffer_len, get_md_oe_state_t)
