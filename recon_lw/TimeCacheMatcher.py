...
class TimeCacheMatcher:
    ...
    def process_objects_batch(self, batch: list) -> None:
        ...
        if stream_time is not None:
            horizon = stream_time["epochSecond"] - self._horizon_delay_seconds
            expiring_times = [t for t in self._time_index2.keys() if t < horizon]
            for t in expiring_times:
                keys = self._time_index2.pop(t)
                for k in keys:
                    match = self._match_index.pop(k)
