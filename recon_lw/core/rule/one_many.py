...
from recon_lw.matching.init_function import (
    MatcherContextProvider,
    SimpleMatcherContext,
    DefaultMatcherContextProvider,
)
...
class OneManyRuleConfig(AbstractRule):
    ...
    def from_dict(name: str, config: dict) -> "OneManyRuleConfig":
        ...
        rule.flush_func = config.get(
            "flush_func", DefaultFlushFunction(config["interpret_func"], last_state_matcher)
        )
