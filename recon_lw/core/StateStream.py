...
class StateStream:
    def __init__(
        self,
        get_next_update_func: Callable[
            [Any], Tuple[str, Th2Timestamp, str, Any]
        ],  # Shoudl return (key, ts, op, state)
        ...
        for tpl in self.state_updates(stream, self._snapshots_collection):
            ...
            if action == "c":
                ...
                if key not in snapshot:
                    ...
                    updated_snapshots.add(snap_id)
                    last_ts = ts
            elif action == "cu":  # Create or update.
                state = self._state_transition_func(state, snapshot.get(key))
                snapshot[key] = state
                updated_snapshots.add(snap_id)
                last_ts = ts
            elif action == "dd":  # Delete if present.
                if key in snapshot:
                    snapshot.pop(key)
                    updated_snapshots.add(snap_id)
                    last_ts = ts
    ...
    def create_snapshot_event(self, ts, snap_id, snapshot_source):
        return self._events_saver.create_event(
            "Snapshot",
            "Snapshot",
            True,
...
def get_next_update_oe(m):
    ...
    ord_status = (
        m["body"]["fields"]["OrdStatus"] if is_fix else str(m["body"]["fields"]["OrderStatus"])
    )
    ord_type = m["body"]["fields"]["OrdType"] if is_fix else str(m["body"]["fields"]["OrderType"])
    target_comp_id = (
        m["body"]["fields"]["header"]["TargetCompID"]
        if is_fix
        else str(m["body"]["fields"]["CompID"])
    )
    ...
    if exec_type == "0":
        ...
        op = "u"
    status = {
        "s": str(m["body"]["fields"]["Side"]),
        "p": float(m["body"]["fields"]["Price"]),
        "q": int(m["body"]["fields"]["LastQty"]) if is_fix else m["body"]["fields"]["LastQuantity"],
        "u_s": unelected_stop,
        "tr": target_comp_id,
        "sec": sec_id,
        "u_t": exec_type,
...
def get_ts_security_md_stream(o):
    ts = None
    seq_id = None
    # ***REMOVED***
    # ***REMOVED***
...
def md_oe_state_stream_compare(match, custom_settings, create_event, save_events):
    if match[1] is None:
        error_event = create_event(
            "OEStateNotFound", "OEStateNotFound", False, {"md_event": match[0]}
        )
...
def create_oe_snapshots_streams(oe_streams, result_events_path, buffer_len=100):
    ...
    state_stream = StateStream(
        get_next_update_oe, get_snapshot_id_oe, state_transition_oe, events_saver
    )

    processor = LastStateMatcher(
        180,
        get_ts_security_md_stream,  # search_ts_key
        get_ts_security_oe_state_stream,  # state_ts_key_order
        md_oe_state_stream_compare,  # interpret
        {},
    ...
    while len(streams) > 0:
        next_batch_len = streams.get_next_batch(message_buffer, buffer_len, get_md_oe_state_t)
