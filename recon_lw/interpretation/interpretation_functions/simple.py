...
class BasicInterpretationFunctionProvider(InterpretationFunctionProvider):
    ...
    def __init__(
            ...
            event_name_provider: Optional[Union[ReconEventNameProvider, ReconEventNameProviderProtocol]]=None,
            recon_event_chain_enhancement: Optional[ReconEventChainEnhancement]=None,
    ...
    def provide(self) -> InterpretationFunctionType:
        def interpret(match_msgs: List[Message], _, event_sequence: dict):
            ...
            if original is not None and copy is not None:
                events += self.match_event_handling_strategy(match_msgs, event_sequence, False, self._original_stream_adapter, self._copy_stream_adapter)
            elif original is None and copy is not None:
                events += self.miss_event_handling_strategy(match_msgs, event_sequence, True, self._original_stream_adapter, self._copy_stream_adapter)
            elif copy is None and original is not None:
                events += self.miss_event_handling_strategy(match_msgs, event_sequence, False, self._original_stream_adapter, self._copy_stream_adapter)
