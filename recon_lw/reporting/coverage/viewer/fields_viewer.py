...
class FieldsTableViewer:
    ...
    def get_fields_table(
        table_name: str, fields: List[Dict[str, Any]], style_config: FieldsTableStyleConfig
    ):
        ...
        for column in columns:
            table += (
                f'<td style="{style_config.row_style}">'
                f'<b style="font-size: {style_config.row_font_size}">{column}</td>'
            )
        table += "</tr>"

        table += "\n".join([get_row(item) for item in fields])
...
class ReconMetadataFieldsViewer:
    def __init__(
        self, recon_context: ReconContext, styles: FieldsTableStyleConfig = FieldsTableStyleConfig()
    ):
