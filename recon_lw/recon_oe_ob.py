...
def report_add_order(er, v: int, create_event):    
    body = {"operation": "ob_add_order",
            "operation_params": {"instr": get_instrument_id(er),
                                ...
                                "side": get_side(er),
                                "str_time_of_event": recon_lw.ts_to_epoch_nano_str(get_transact_time(er))},
                                "otv": v}
    return create_event("OEMDImage", "OEMDImage", True, body)


def report_delete_order(er, v: int, create_event, str_time_of_event= None):
    tov = recon_lw.ts_to_epoch_nano_str(get_transact_time(er)) if str_time_of_event is None else str_time_of_event
...
def oe_report_md_images(update_states, create_event, send_events):
    ...
    if len(update_states) == 1:
        ...
        if mm_er["ExecType"] == "0" and get_order_type(er) not in ["3","4"]:
            ...
            if get_resting_price(prev_er) == get_resting_price(er) and get_resting_qty(prev_er) > get_resting_qty(er):
                ev = report_update_order(er,0,create_event)
                ev["attachedMessageIds"] = [er["messageId"]]
                ev["scope"] = er["sessionId"]
                ev["body"]["timestamp"] = er["timestamp"]
                send_events([ev])
            elif get_resting_price(prev_er) != get_resting_price(er) or get_resting_qty(prev_er) < get_resting_qty(er):
                ev1 = report_delete_order(prev_er,0,create_event,str_time_of_event=recon_lw.ts_to_epoch_nano_str(get_transact_time(er)))
        ...
        if mm_er["ExecType"] == "0":
            ...
            prev_er = prev_state["last_er"]
            last_er = update_states[-1][0]
            ev1 = report_delete_order(prev_er,0,create_event,str_time_of_event=recon_lw.ts_to_epoch_nano_str(get_transact_time(last_er)))
            ...
            #unexpected event
            ev = create_event("OEMD_Unexpected", "OEMD_Unexpected", True, update_states_synopsis(update_states))
            ev["attachedMessageIds"] = [upd[0]["messageId"] for upd in update_states]
            send_events([ev])
...
def update_states_synopsis(update_states):
    prev_state = update_states[0][1]
    prev_state_syn = "no_state" if prev_state["no_state"] else get_er_synopsis(prev_state["last_er"])
