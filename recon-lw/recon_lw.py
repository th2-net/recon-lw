...
def split_messages_pickle_for_recons(message_pickle_path, output_path, sessions_list):
    ...
    for s in sessions_list:
        ...
        arr = list(map(simplify_message, messages_session_in))
        ...
        arr = list(map(simplify_message, messages_session_out))
        arr.sort(key=lambda m: time_stamp_key(m["timestamp"]))
...
if __name__ == '__main__':
    messages = Data.from_cache_file("path/to/messages/pickle.pickle")
    result = message_utils.get_totals(messages,
                                      [lambda m: m["sessionId"]],
                                      lambda m: protocol(m) in ["FIX","PROTOCOL"])
    split_messages_pickle_for_recons("temp/recon_lw/messages.pickle",
                                     "temp/recon_lw/spl_msgs",
                                     result.keys())
