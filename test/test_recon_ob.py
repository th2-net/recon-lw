...
class ReconObTests(unittest.TestCase):
    def test_sequence_cache_add(self):
        sequence_cache = {
            "sequence": SortedKeyList(),
            "times": SortedKeyList(),
            "duplicates": SortedKeyList(),
        }
        # Add first message
        seq = 1
        ...
        self.assertEqual(
            sequence_cache["sequence"][0],
            (seq, message1),
            "Fail to add first message in sequence cache",
        )
        self.assertEqual(
            sequence_cache["times"][0],
            (timestamp, seq),
        ...
        self.assertEqual(
            sequence_cache["duplicates"][0],
            (seq, message2["messageId"], message1["messageId"]),
        ...
        self.assertEqual(
            sequence_cache["sequence"][1],
            (seq, message3),
            "Fail to add second message in sequence cache",
        )
        self.assertEqual(
            sequence_cache["times"][1],
            (timestamp, seq),
